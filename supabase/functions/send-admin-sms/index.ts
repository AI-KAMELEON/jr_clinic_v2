...
async function getAccessToken() {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Błąd przy pobieraniu tokena: ${resp.status} ${resp.statusText} → ${errorMsg}`);
  }
  const data = await resp.json();
  return data.access_token;
}
// Wysyłanie SMS Play
async function sendSms(token, to, text) {
  ...
  try {
    ...
    for (const v of visits){
      ...
      if (type === "PRZYPOMNIENIE") {
        text = `Dzień dobry,
Przypominamy o jutrzejszej wizycie stomatologicznej,
Godzina: ${visitTime},
JR Clinic,
Rynek 7,
Maków Mazowiecki`;
